...
{
	/// <summary>
	/// A SignalR channel for communicating Processor results to the Distenka API.
	/// </summary>
	public class BroadCastChannel
	{
		readonly Uri baseUri;
		readonly IAuthenticationProvider auth;
		readonly ILogger<BroadCastChannel> logger;
		...
		/// <summary>
		/// Initializes a new <see cref="BroadCastChannel"/>.
		/// </summary>
		...
		public BroadCastChannel(Uri baseUri, IAuthenticationProvider auth, ILogger<BroadCastChannel> logger)
		...
		public BroadCastChannel(Uri baseUri, IAuthenticationProvider auth, ILogger<BroadCastChannel> logger, Action<HttpConnectionOptions> opts)
		{
		...
		public class Connection : IAsyncDisposable
		{
			...
			public Connection(IResultsConnection conn, ILogger<BroadCastChannel> logger)
			{
			...
			/// <summary>
			/// Call made when the node observes the Processor app starting.
			/// </summary>
			/// <param name="pid">The local process ID of the Processor app.</param>
			public Task StartApp(int pid) => SendPriorityAsync(nameof(StartApp), pid);

			/// <summary>
			/// Call made when the node aborts the starting of a Processor because of a user-requested cancellation during acquisition.
			/// </summary>
			/// <returns></returns>
			public Task Abort() => SendPriorityAsync(nameof(Abort));

			/// <summary>
			/// Call made when the node observes the Processor app exiting.
			/// </summary>
			/// <param name="exitCode">The exit code of the Processor app process.</param>
			/// <param name="timedOut">Indicates whether the process timed out.</param>
			/// <param name="duration">The amount of time the Processor app was running.</param>
			public Task ExitApp(int exitCode, bool timedOut, long duration) =>
				SendPriorityAsync(nameof(ExitApp), exitCode, timedOut, duration);

			/// <summary>
			/// Call made when an unexpected error happens on the node while running a Processor.
			/// </summary>
			...
			/// <summary>
			/// Call made by the Processor execution to set the expected total number of items.
			/// </summary>
			...
			/// <summary>
			/// Call made by the Processor execution to broadcast an update of state and item progress.
			/// </summary>
			/// <param name="state">The state of the Processor.</param>
			/// <param name="categories">The progress of the Processor.</param>
			...
			/// <summary>
			/// Call made by the Processor execution when a Processor method has been invoked.
			/// </summary>
			/// <param name="outcome">The outcome of invoking the Processor method.</param>
			public Task MethodResult(MethodOutcome outcome) => SendPriorityAsync(nameof(MethodResult), outcome);

			/// <summary>
			/// Call made by the Processor execution when an item has been processed.
			/// </summary>
			...
			/// <summary>
			/// Call made by the node when data is read from the standard output or error of the Processor app.
			/// </summary>
			...
			/// <summary>
			/// Call made by the Processor execution when the Processor is complete.
			/// </summary>
			/// <param name="disposition">The <see cref="Disposition"/> of the Processor execution.</param>
			...
			/// </summary>
			/// <param name="disposition">The <see cref="Disposition"/> of the Processor execution.</param>
			/// <param name="output">The output from <see cref="IProcessor.FinalizeAsync(Disposition)"/>.</param>
	...
	public interface IResultsConnection : IAsyncDisposable
	{
		...
		/// <summary>
		/// Occurs when the Microsoft.AspNetCore.SignalR.BroadCast.HubConnection starts reconnecting
	...
	public class HubConnectionWrapper : IResultsConnection
	{
		...
		/// <summary>
		/// Occurs when the Microsoft.AspNetCore.SignalR.BroadCast.HubConnection successfully
		/// reconnects after losing its underlying connection.
		/// </summary>
		/// <remarks>
		/// The System.String parameter will be the Microsoft.AspNetCore.SignalR.BroadCast.HubConnection's
