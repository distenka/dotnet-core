...
{
	/// <summary>
	/// Logs results from a <see cref="IProcessor"/>.
	/// </summary>
	public class ResultLog
	{
		...
		/// </summary>
		/// <remarks>
		/// <see cref="TotalItemCount"/> will be null when a <see cref="IProcessor"/> sets canBeCounted to false when using ToAsyncEnumerable. The difference between
		/// this total and the sum of <see cref="SuccessfulItemCount"/> and <see cref="FailedItemCount"/> is the number of items
		/// yet to be processed or not processed in the case of a Processor that ended in the <see cref="Disposition.Failed"/> state.
		/// </remarks>
		...
		/// Initializes a new <see cref="ResultLog"/>.
		/// </summary>
		public ResultLog(IEnumerable<ItemResult> items, Disposition disposition, int? totalItemCount, Dictionary<ProcessorMethod, MethodOutcome> methods, object output)
		{
		...
		/// <summary>
		/// Produces a report of this Processor for the console.
		/// </summary>
		public override string ToString()
		{
			...
			report.AppendLine($"Processor {Disposition.ToString().ToUpper()}");
			report.AppendLine(ConsoleFormat.DoubleLine);
			report.AppendLine();
