...
namespace Distenka.Tests.Scenarios.Results
{
	public class ChannelTests : IAsyncDisposable
	{
		...
		readonly BroadCastChannel.Connection channel;
		readonly int minDelay, maxDelay;

		public ChannelTests()
		{
			connection = new FakeResultsConnection();
			channel = new BroadCastChannel.Connection(connection, new FakeLogger<BroadCastChannel>());

			minDelay = BroadCastChannel.Connection.MinDelay + 200;
			maxDelay = BroadCastChannel.Connection.MaxDelay + 200;
		}
